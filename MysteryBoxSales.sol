...
contract MysteryBoxSales is ERC721 {
    ...
    event BlockHash(uint256 indexed revealTime, bytes32 hash);
    ...
    function gift(address recipient, uint256 _saleId) public payable {
        ...
        emit MysteryBoxBought(_saleId, numParticipants, recipient);
    ...
    function withdrawTokensInBatch(uint256 _saleId, uint256[] calldata _participantIndices) external {
        ...
        for(uint256 i = 0; i < numIndices; i++) {
            ...
            sales[_saleId].participants[_participantIndex] = address(0);
            {
                ...
                emit MysteryBoxWithdrawn(_saleId, _participantIndex, participant);
            }
